...
public interface CollectionMapper extends CompositeMapper<Collection> {
  @Select("SELECT COUNT(0) FROM xzyy_collection WHERE userId = #{userId} AND girlId = #{girlId}")
  int countByUserIdAndGirlId(@Param("userId") String userId, @Param("girlId") String girlId);

  @Delete("DELETE FROM xzyy_collection WHERE userId = #{userId} AND girlId = #{girlId}")
