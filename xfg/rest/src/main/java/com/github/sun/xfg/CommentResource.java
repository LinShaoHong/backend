...
import com.github.sun.xfg.util.Dates;
...
public class CommentResource extends AbstractResource {
  ...
  public SingleResponse<MessageRes> getReplyInfo(@QueryParam("start") int start,
    ...
    try {
      SqlBuilder sb = factory.create();
      Expression condition = sb.field("replierId").eq(user.getId())
        .or(sb.field("commentatorId").eq(Comment.SYSTEM).and(sb.field("replierId").isNull()));
      if (latestId != null) {
      ...
      int unReads = latestId != null ? 0 :
        mapper.countByTemplate(sb.from(Comment.class)
          .where(sb.field("replierId").eq(user.getId()).and(sb.field("read").eq(false)))
          .count()
          .template());
