...
public interface CommentMapper extends CompositeMapper<Comment> {
  @Select("SELECT * FROM xfg_comment WHERE sessionId IN(${sessionIds})")
  List<Comment> findBySessionIdIn(@Flatten @Param("sessionIds") List<String> sessionIds);

  @Update("UPDATE xfg_comment SET likes = likes + 1 WHERE id = #{id}")
  void incLikes(@Param("id") String id);

  @Update("UPDATE xfg_comment SET hates = hates + 1 WHERE id = #{id}")
  void incHates(@Param("id") String id);

  @Update("UPDATE xfg_comment SET `read` = TRUE WHERE id = #{id}")
  void read(@Param("id") String id);

  @Update("UPDATE xfg_comment SET `read` = TRUE WHERE replierId = #{userId}")
  void readAll(@Param("userId") String userId);

  @Select("SELECT id FROM xfg_comment WHERE commentatorId = 'SYSTEM' AND replierId IS NULL")
  Set<String> findAllSystemMessageId();

  @Select("select m.rowNum from " +
    "(SELECT @rownum := @rownum + 1 as rowNum, e.* FROM (SELECT @rownum := 0) r, xfg_comment e " +
